...
//----------------------------------------------------------------------
// Interrupt::RemovePendingTimerInterrupt
//      Remove the pending timer interrupt from the head of the pending list.
//      This is used when a thread finishes to reset the timer quantum.
//----------------------------------------------------------------------
void
Interrupt::RemovePendingTimerInterrupt()
{
    if (!pending->IsEmpty()) {
        int when;
        PendingInterrupt *toRemove = (PendingInterrupt *)pending->SortedRemove(&when);
        
        if (toRemove != NULL && toRemove->type == TimerInt) {
            DEBUG('i', "Removing pending timer interrupt scheduled at time %d\n", when);
            delete toRemove;
        } else if (toRemove != NULL) {
            // If it's not a timer interrupt, put it back
            pending->SortedInsert(toRemove, when);
        }
    }
}
