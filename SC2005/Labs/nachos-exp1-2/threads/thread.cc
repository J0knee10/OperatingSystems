...
Thread::Finish ()
{
	...
	/* Add code here to reset the timer interrupt so that the next 
        ...
        */
        if (timer != NULL && interrupt != NULL) {
            // Remove existing timer interrupt from pending list
            timer -> Reset();
        }

	Sleep();					// invokes SWITCH
    } else {					// this thread will be joined
	DEBUG('t', "Finishing thread %s #%i that will be joined, so wait until Join has been called\n", getName(), pid);
